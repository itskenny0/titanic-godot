...
INDEX = ROOT / "native/restoration/Sources/GameFileIndex.swift"
GOG_PROFILE = ROOT / "scripts/gog-local-profile.json"
KEYS = {
...
def requirements() -> dict[int, tuple[str, ...]]:
    ...
    blocks = re.findall(r'(\d+): Set\("""(.*?)"""', INDEX.read_text(), re.S)
    result = {int(disc): tuple(sorted(block.split())) for disc, block in blocks}
...
def run(args: argparse.Namespace) -> dict:
    ...
    if args.installer:
        ...
        if not args.dry_run:
            publish(files, output, digital=report["layout"] == "gog-local")
    if not args.dry_run:
        report.update(status="prepared", output=str(output), next_step="In Titanic.app choose the prepared folder containing cd1 and cd2.")
    return report
