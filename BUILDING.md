...
CI uses synthetic fixtures, including tiny generated ISOs; it needs no commercial game files. Set `TAOOT_ISO_DIR` locally to check your own disc images. The gameplay fixtures follow the pinned reference engine. With owned game files, run `TAOOT_GAME_DATA=/path/to/gamedata go test ./internal/...` for startup, navigation, and save/load integration. Set `TAOOT_MOD_DATA` to the extracted Extended Mod directory to check its startup too. Set `TAOOT_ADAPTIVE_SAVE` to a normal exploration `.ti` save to test side panels, item selection and dragging an item back into the bag without changing the save. The Godot integration test uses actual game data and the UI bridge; `Dummy` audio checks do not verify sound on a device.
